...
def get_conversational_rag_chain(retriever_chain): 
    ...
    prompt = ChatPromptTemplate.from_messages([(
        ...
        If the information is not available on provided specific website, use crew ai agents
        """),
...
crew = Crew(
    ...
    memory=True,  # Retain memory across tasks
    cache=True,  # Enable caching for faster responses
    max_rpm=100,  # Max requests per minute
    share_crew=True  # Agents can share data and insights with each other
)
...
async def get_response(user_input: UserInput):
    ...
    # Validate the input to ensure URLs are not directly queried
    ...
    try:
        # Invoke the conversational RAG chain to check the website's content
        response = conversation_rag_chain.invoke({
            "chat_history": session_data["chat_history"],
            "input": user_input.question
        })

        # Check if no relevant answer was found from the website
        if response['answer'].strip() == "":
            logger.info("No relevant answer found, falling back to AI agents for reasoning.")
            # Send the fallback message to UI
            fallback_message = "No relevant answer found, falling back to AI agents for reasoning."
            
            # Use the chat history to generate reasoning with agents
            if session_data["chat_history"]:
                last_question = session_data["chat_history"][-1]['content']
                logger.info(f"Using last question for reasoning: {last_question}")
                
                # Pass the question to the crew for AI agents' reasoning
                result = crew.kickoff(inputs={'question': last_question})
                
                # Assuming the crew generates an article as the fallback answer
                fallback_response = result.get('output', 'No output generated by the agents')
                
                # Update session history with agent's response
                ...
                # Return the fallback message along with the response from the AI agents
                return {
                    "answer": fallback_message,  # Fallback message to UI
                    "agent_response": fallback_response  # Agent response to show
                }
        else:
            # Update session history with website-based response
            session_data["chat_history"].append({
                "role": "user", 
                "content": user_input.question
            })
            ...
            # Return the answer based on website content
            return {"answer": response['answer']}
    
    except Exception as e:
        logger.error(f"Error in processing user input: {e}")
